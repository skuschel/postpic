...
class Axis(object):
    ...
    def __init__(self, name='', unit='', **kwargs):
        ...
        if self._extent is not None:
            if not isinstance(self._extent, collections.abc.Iterable) or len(self._extent) != 2:
...
def _reducing_numpy_method(method):
    ...
    def new_method(self, axis=None, out=None, keepdims=None, **kwargs):
        ...
        if not isinstance(axisiter, collections.abc.Iterable):
            axisiter = (axis,)
...
class Field(NDArrayOperatorsMixin):
    ...
    * by not passing anything, which will create default axes from 0 to 1.
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        if not out:
            if method == '__call__':
                ...
                if not isinstance(reduceaxis, collections.abc.Iterable):
    ...
    def pad(self, pad_width, mode='constant', **kwargs):
        ...
        if not isinstance(pad_width, collections.abc.Iterable):
            pad_width = [pad_width]
        ...
        for i, axis_pad in enumerate(pad_width):
            if not isinstance(axis_pad, collections.abc.Iterable):
    ...
    def ensure_transform_state(self, transform_states):
        ...
        """
        if not isinstance(transform_states, collections.abc.Mapping):
            if not isinstance(transform_states, collections.abc.Iterable):
    ...
    def shift_grid_by(self, dx, interpolation='fourier'):
        ...
        if not isinstance(dx, collections.abc.Mapping):
